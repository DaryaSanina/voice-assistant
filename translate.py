...
def translate_text(user_message_text: str) -> str:
    ...
    if len(languages) == 0:  # Translate from automatically detected language into English
    ...
    else:  # If the user has stated both languages
        translation = translator.translate(text, src=languages[0], dest=languages[1])
...
def get_language_names_from_text(text) -> list:
    ...
    for entity in doc.ents:  # Iterate through all recognized entities (like names, places etc.)
        if entity.label_ == 'NORP' or entity.label_ == 'LANGUAGE':  # The entity is a language
